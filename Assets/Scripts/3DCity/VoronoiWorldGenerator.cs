...
public class VoronoiWorldGenerator : MonoBehaviour {
    ...
    private const float EdgeY = 0.06f; // Position edges slightly above terrain for seamless fit
    private const float PolygonInset = 0.3f; // Shrink terrain polygons inward to fit edges
    ...
    private Mesh BuildPolygonMesh(List<Vector3> polygon) {
        // Inset polygon towards center, then set Y = 0.05
        var insetPolygon = InsetPolygon(polygon, PolygonInset);
        var verts = insetPolygon
    ...
    private List<Vector3> InsetPolygon(List<Vector3> polygon, float inset) {
        if (polygon.Count < 3 || inset <= 0)
            return polygon;
        ...
        // Move each vertex towards centroid by inset amount
        var insetPoly = new List<Vector3>();
        foreach (var v in polygon) {
            Vector3 direction = (centroid - v).normalized;
            insetPoly.Add(v + direction * inset);
        }
    ...
    private void CreateEdgeObject(int regionA, int regionB) {
        ...
        // Add renderer with visible material (always visible)
    ...
    /// <summary>Gets all edges adjacent to a region.</summary>
    public List<VoronoiEdge> GetEdgesForRegion(int regionId) {
        var regionEdges = new List<VoronoiEdge>();
        foreach (var edge in edges) {
    ...
    public void ColorEdgesForTerrain(int regionId, TerrainType terrainType) {
        var adjacentEdges = GetEdgesForRegion(regionId);

        foreach (var edge in adjacentEdges) {
            if (!edge.IsAssigned) {
                edge.SetTerrainColor(terrainType);
            }
        }
    }
}
