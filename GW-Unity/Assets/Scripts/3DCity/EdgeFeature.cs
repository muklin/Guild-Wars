...
public class EdgeFeature
{
    ...
    private bool ValidateCliff()
    {
        ...
        // If 2 edges, they must be contiguous (share a region)
        if (Edges.Count == 2)
        {
            var (a1, b1) = Edges[0];
            var (a2, b2) = Edges[1];

            // Check if edges share a region
            bool shareRegion = (a1 == a2 || a1 == b2 || b1 == a2 || b1 == b2);
            if (!shareRegion)
            {
                Debug.LogWarning("Invalid cliff: edges must be contiguous (share a region)");
                return false;
            }
        }
