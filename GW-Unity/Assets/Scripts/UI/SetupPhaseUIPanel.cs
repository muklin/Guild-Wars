...
/// <summary>
/// UI Panel for Setup Phase: step-by-step city and guild creation.
///
/// Steps shown:
...
/// </summary>
public class SetupPhaseUIPanel : UIPanel {
    // ─── Shared ───────────────────────────────────────────────────────
    [Header("Shared")]
    ...
    protected void Awake() {
        SetupPhase = SetupPhaseManager.Instance?.Phase;
        if (SetupPhase == null)
            Debug.LogWarning("[SetupPhaseUIPanel] SetupPhase not found — ensure SetupPhaseManager exists.");
    }

    public override void Initialize() {
        // Call base to set up basic layout
        base.Initialize();

        // Position on the right side of screen (doesn't block Terrain UI on left)
        rectTransform.anchorMin = new Vector2(1, 0);
        rectTransform.anchorMax = new Vector2(1, 1);
        rectTransform.offsetMin = new Vector2(-400, 0);
        rectTransform.offsetMax = Vector2.zero;

        // Make background semi-transparent and block raycasts
    ...
    private void UpdateStepTitle() {
        if (stepTitleText == null)
            return;
        stepTitleText.text = SetupPhase.CurrentStep switch {
            SetupPhaseStep.Terrain => "Step 1: Terrain Setup",
            SetupPhaseStep.DistrictSetup => "Step 2: District Setup",
            SetupPhaseStep.HQPlacement => "Step 3: Place Headquarters",
            SetupPhaseStep.GuildSetup => "Step 4: Guild Setup",
            SetupPhaseStep.Complete => "Setup Phase Complete",
            _ => "Setup Phase"
        };
    }
    ...
    private void UpdateLog() {
        if (logText == null)
            return;
        var log = SetupPhase.SetupLog;
        int start = Mathf.Max(0, log.Count - 20);
        logText.text = string.Join("\n", log.GetRange(start, log.Count - start));
    }

    private void ShowStepPanel() {
        SetActive(terrainPanel, SetupPhase.CurrentStep == SetupPhaseStep.Terrain);
        SetActive(districtSetupPanel, SetupPhase.CurrentStep == SetupPhaseStep.DistrictSetup);
        SetActive(hqPanel, SetupPhase.CurrentStep == SetupPhaseStep.HQPlacement);
        SetActive(guildSetupPanel, SetupPhase.CurrentStep == SetupPhaseStep.GuildSetup);

        if (SetupPhase.CurrentStep == SetupPhaseStep.DistrictSetup)
            UpdateDistrictTurnUI();

        if (SetupPhase.CurrentStep == SetupPhaseStep.HQPlacement)
            RefreshHQDropdown();
    }

    private void UpdateDistrictTurnUI() {
        bool isPlayerTurn = SetupPhase.IsPlayerTurn();
    ...
    private void OnPlaceTerrainClicked() {
        ...
        bool ok = SetupPhase.PlaceTerrainFeature((TerrainType)typeIdx, desc, cellIdx % 5, cellIdx / 5);
        if (ok) {
            ShowStatus($"{(TerrainType)typeIdx} placed.", Color.green);
            if (terrainDescriptionInput)
                terrainDescriptionInput.text = "";
            Refresh();
        }
    }
    ...
    private void OnDoneTerrainClicked() {
        if (SetupPhase == null)
            return;
        bool ok = SetupPhase.FinishTerrainPlacement();
        if (!ok)
            ShowStatus("Place at least one terrain feature first.", Color.red);
        else
            Refresh();
    }
    ...
    private bool ConfirmPlaceDistrict() {
        ...
        if (consumes.Count(c => !string.IsNullOrEmpty(c)) < 3) { ShowStatus("Enter at least 3 consumed resources.", Color.red); return false; }

        bool ok = SetupPhase.PlaceDistrict(name, cls, faction, produces, consumes, cellIdx % 5, cellIdx / 5, walled);
        if (!ok)
            ShowStatus("Could not place district.", Color.red);
        else
            ShowStatus($"District '{name}' placed.", Color.green);
        return ok;
    }
    ...
    private bool ConfirmPlaceThreat() {
        ...
        bool ok = SetupPhase.PlaceThreat(name, desc, dc, mitigation, cellIdx % 5, cellIdx / 5);
        if (!ok)
            ShowStatus("Could not place threat.", Color.red);
        else
            ShowStatus($"Threat '{name}' placed.", Color.green);
        return ok;
    }
    ...
    private bool ConfirmAddTrade() {
        ...
        bool ok = SetupPhase.AddTradingDestination(name, produces, consumes);
        if (!ok)
            ShowStatus("Could not add trading destination.", Color.red);
        else
            ShowStatus($"Trading destination '{name}' added.", Color.green);
        return ok;
    }
    ...
    private bool ConfirmDefineLeadership() {
        ...
        SetupPhase.DefineCityLeadership(head, succession);
    ...
    private void OnPlaceHQClicked() {
        if (SetupPhase == null || hqDistrictOptions.Count == 0)
            return;
        int idx = hqDistrictDropdown?.value ?? 0;
        if (idx >= hqDistrictOptions.Count)
            return;

        bool ok = SetupPhase.PlacePlayerHQ(hqDistrictOptions[idx].Id);
        if (ok) {
    ...
    private void OnStartGameClicked() {
        ...
        bool ok = SetupPhase.CreatePlayerGuild(guildName, leaderName, leaderClass, secondName, secondClass);
        if (ok) {
    ...
    private void ShowStatus(string message, Color color) {
        ...
        Debug.Log($"[SetupPhaseUI] {message}");
    }
