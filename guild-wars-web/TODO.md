...
- SetupPhase.js kitchen-sink split: DONE (2026-07-19). All 4 modules extracted: `GroundplaneAudit.js` (28 methods, zero cross-module calls — holds `this.sp` only for `gameStateManager`/`worldGenerator`), `TerrainSetup.js`, `DistrictSetup.js`, `StreetBlockPlotPipeline.js` (10/32/15 methods respectively, heavily cross-coupled — confirmed via full dependency analysis, ~26 real cross-bucket calls). `SetupPhase.js` is now a 320-line thin orchestrator: constructor wires all 4 module instances, every moved method gets a one-line delegate (`name(...args) { return this.module.name(...args) }`), cross-bucket calls inside the modules route through `this.sp.methodName()` rather than direct sibling references. 6 static class members (`NORTH_HALF_ANGLE_DEG`, `ALL_DISTRICT_TYPES`, `_shuffleSeeded`, `INELIGIBLE_PROMOTION_TERRAIN_TYPES`, `_isIneligibleTerrainPlot`, `producedAmount`) and a standalone character/recruit-generation subsystem relocated to plain module-level consts/functions in their owning file.
  - Caught mid-flight by careful boundary validation + a live smoke test (not static analysis alone): a false-positive method-boundary match that nearly mis-scoped `TerrainSetup`, 6 missed `static` class members that got silently swallowed into the wrong method's body, an initial fix that left those statics as instance fields/methods instead of module-level consts/functions (ReferenceError on first live run), and one function (`computeTerrainPlotStreetAdjacency`) actually shared across 2 buckets, not 1 — now exported from `StreetBlockPlotPipeline.js` and imported by `TerrainSetup.js`. All fixed and re-verified; a full live in-memory run (initialize → assign terrain → generate districts → assign types → finishSubdivision incl. auto-assign → createPlayerGuild → worldbuilding → serialize/deserialize round-trip) passes end to end, plus the full existing test suite (0 regressions).
